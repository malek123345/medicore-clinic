...
<div class="scene">
  ...
  <div class="card" [class.signup-mode]="tab()==='signup'">
    ...
    <div class="right">
      ...
      <div class="form-panel" [class.fp-hidden]="tab()==='signup'">
        <div class="form-head">
          <div class="form-eyebrow"><span class="eyebrow-dot"></span>Espace sécurisé</div>
        ...
        <form [formGroup]="loginForm" (ngSubmit)="submitLogin()">
          ...
          <button type="submit" class="btn-submit" [class.btn-loading]="loading()">
            ...
            @else { Se connecter <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg> }
          </button>
        </form>
      ...
      <div class="form-panel" [class.fp-hidden]="tab()==='login' || signupStage()==='otp' || signupStage()==='done'">
        ...
        <form [formGroup]="signupForm" (ngSubmit)="submitSignupStep1()">
          ...
          <div class="fg">
            <label class="fl">NUMÉRO DE TÉLÉPHONE *</label>
          ...
          <p class="terms-txt">En vous inscrivant, vous acceptez nos <a href="#">Conditions d'utilisation</a></p>
        </form>
      ...
      <div class="form-panel otp-panel" [class.fp-hidden]="tab()==='login' || signupStage()!=='otp'">
        ...
        <div class="otp-header">
          ...
          <h2 class="otp-title">Vérification Email</h2>
          <p class="otp-desc">
            Un code de vérification à <strong>6 chiffres</strong> a été envoyé à<br/>
            <span class="otp-phone-num">{{ signupForm.get('email')?.value }}</span>
          </p>
        </div>
      ...
      <div class="form-panel done-panel" [class.fp-hidden]="tab()==='login' || signupStage()!=='done'">
        <div class="done-wrap">
          ...
          <p class="done-desc">
            ...
            Votre email <strong>{{ signupForm.get('email')?.value }}</strong><br/>
  ...
  styles: [`
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    :host {
      --P:#1d5fe0; --P2:#154dc8; --violet:#7c3aed; --em:#10b981; --rose:#f0426a;
      --brd2:#e2e9f8; --txt:#07193b; --txt3:#526080; --txt4:#8ba0bf;
      --Pl:rgba(29,95,224,0.08); --Pl2:rgba(29,95,224,0.14);
    }
    .scene { position:relative;width:100%;min-height:100vh;display:flex;align-items:center;justify-content:center;overflow:hidden;padding:24px;background:linear-gradient(150deg,#060f2a 0%,#0d1f5c 35%,#1040b0 65%,#0a2575 100%); }
    ...
    .card { position:relative;z-index:10;display:flex;width:940px;max-width:100%;border-radius:26px;overflow:hidden;box-shadow:0 50px 120px rgba(0,0,0,.55),0 0 0 1px rgba(255,255,255,.1);animation:cardIn .8s cubic-bezier(.22,1,.36,1) both; }
    @keyframes cardIn { from{opacity:0;transform:translateY(36px) scale(.97)} to{opacity:1;transform:none} }
    .left { width:42%;flex-shrink:0;background:linear-gradient(168deg,#0a1840 0%,#0d2a6e 45%,#1040b0 80%,#1555c8 100%);display:flex;flex-direction:column;padding:40px 36px;position:relative;overflow:hidden; }
    ...
    .btn-submit { position:relative;overflow:hidden;width:100%;padding:13px;border:none;border-radius:13px;background:linear-gradient(135deg,var(--P),var(--P2));color:white;font-family:'Plus Jakarta Sans',sans-serif;font-size:14px;font-weight:700;cursor:pointer;letter-spacing:.1px;box-shadow:0 8px 24px rgba(29,95,224,.35);transition:all .25s cubic-bezier(.34,1.56,.64,1);display:flex;align-items:center;justify-content:center;gap:9px; }
    ...
    .otp-box { width:52px;height:62px;border:2px solid #e4eaf5;border-radius:14px;background:#f7f9fd;text-align:center;font-size:24px;font-weight:900;color:var(--txt);font-family:'Plus Jakarta Sans',sans-serif;outline:none;transition:all .2s;caret-color:transparent; }
...
export class LoginComponent implements AfterViewInit, OnDestroy {
  ...
  // ✅ إرسال للباكند — يولّد الكود ويبعته بـ email
  async submitSignupStep1() {
    ...
    try {
      await firstValueFrom(this.http.post(`${this.API}/auth/register`, {
      ...
      this.signupError.set(err?.error?.message || 'Erreur lors de l\'inscription.');
      this.loading.set(false);
    }
  }
  ...
  onOtpPaste(event: ClipboardEvent) {
    ...
    setTimeout(() => document.getElementById(`otp-${nextEmpty === -1 ? 5 : nextEmpty}`)?.focus(), 0);
    if (text.length === 6) setTimeout(() => this.verifyOtp(), 300);
  }

  // ✅ التحقق من الكود مع الباكند
  async verifyOtp() {
    ...
    this.loading.set(true);
    try {
      await firstValueFrom(this.http.post(`${this.API}/auth/verify`, {
  ...
  formatPhone(phone: string | null | undefined): string {
    if (!phone) return '';
    return phone.replace(/\D/g, '').replace(/(\d{2})(\d{3})(\d{3})/, '$1 $2 $3');
  }
